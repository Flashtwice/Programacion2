...
public class CandidatoDAO {
	...
	public Candidato[] modificarCandidato(String cedula, String nuevoNombre ,String nuevoApellido,String nuevaCedula,int nuevoEdad,String nuevoCargo,Candidato[] ArrayCandidato) {
		boolean existe = buscarCandidato(cedula, ArrayCandidato);
		if(existe == true){
			for(int i = 0; i < ArrayCandidato.length; i++) {
				if(ArrayCandidato[i]!= null){
					if(ArrayCandidato[i].getCedula().equals(cedula)){				
						ArrayCandidato[i].setNombre(nuevoNombre);
						ArrayCandidato[i].setApellido(nuevoApellido);
						ArrayCandidato[i].setCedula(nuevaCedula);
						ArrayCandidato[i].setCargo(nuevoCargo);
						ArrayCandidato[i].setEdad(nuevoEdad);
						System.out.println("El Candidato se ha modificado correctamente");
					}
				}
			}
